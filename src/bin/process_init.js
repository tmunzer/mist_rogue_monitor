            ...
            if (err) {
                ...
                accounts.forEach(account => {
                    if (!account._token || !account._token.apitoken) {
                        logger.error("account " + account.org_id + " - no API token configured")
                            // TODO: disable site
                    } else if (!account._site || !account._site.configured) {
                        logger.error("account " + account.org_id + " - no site configured")
                            // TODO: disable site
                    } else if (account.disabled) {
                        logger.error("account " + account.org_id + " - disabled")
                            // TODO: disable site
                    } else {
                        logger.info("account " + account.org_id + " - syncing started...")
                        process_sync.run(account, date, (account) => {
                            logger.info("account " + account.org_id + " - syncing finished...")

                            if (!account._alert || !account._alert.configured) {
                                logger.warning("account " + account.org_id + " - alerting not configured...")
                            } else if (!account._alert.enabled) {
                                logger.warning("account " + account.org_id + " - alerting disabled...")
                            } else if (account._alert.to_emails.length == 0) {
                                logger.warning("account " + account.org_id + " - recipients not configured...")
                            } else {
                                logger.info("account " + account.org_id + " - emailing in process...")
